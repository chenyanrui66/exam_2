...
\subsection{模型对比}
将我们方法与传统TDOA方法对比：TDOA用时刻差消去$\tau_0$，方程数比设备数少1，同样的台数下冗余更少，且无法直接给出音爆时刻；我们的TOA模型保留$\tau_0$为未知量，每台设备贡献一个独立方程，冗余度更高，契合异常数据检验的需要。在初值求取环节，我们借用了TDOA的差分思想消去二次项，相当于两种体制的结合。与直接以全部7台数据做最小二乘的朴素做法相比，我们的子集枚举与残差检验避免了D、F异常记录对解的污染——若7台混用，解将被拉向错误位置且难以察觉。
...
本文以音爆到达时间（TOA）传播机理确定可行解，通过蒙特卡洛误差传播标定关联阈值，并利用位置精度因子（PDOP）指导监测台网优化。在计时误差服从
$U(-0.5,+0.5)\,\mathrm{s}$的条件下进行300次蒙特卡洛实验，20台加密台网相较原7台将三维定位误差均值由
...
本文固定取声速$c=340\,\mathrm{m/s}$，未显式考虑温度分层、风场、湿度和大气折射对传播速度及传播路径的影响。论文中的典型台站至残骸水平距离约为
\textbf{$20\sim70\,\mathrm{km}$}。仅从传播距离的比例误差估算，当有效声速偏差为$1\%$时，就可能产生约
\textbf{$200\sim700\,\mathrm{m}$}的距离量级偏差，已接近题目要求的$1\,\mathrm{km}$定位误差上限。

现有300次蒙特卡洛实验主要扰动设备记录时刻和坐标量化误差，尚未对声速、风速和温度梯度开展系统灵敏度分析，因此不能保证模型在复杂大气条件下仍能保持
\textbf{$456.95\,\mathrm{m}$的三维误差95\%分位}。

\item \textbf{模型阈值对噪声分布和台站系统偏差存在一定依赖。}

本文的严阈值是按照独立均匀分布
$U(-0.5,+0.5)\,\mathrm{s}$标定的。当误差变为每个读数均位于$\pm0.5\,\mathrm{s}$边界的两点分布时，严阈值误杀率达到
\textbf{$76\%$}，300次试验中有
\textbf{229次}必须依赖剩余读数补全。

在该极端噪声下，20台方案虽然仍保持
\textbf{$300/300$正确关联}，但三维误差95\%分位由标称情形的
\textbf{$457\,\mathrm{m}$升至$795\,\mathrm{m}$}，仍有
\textbf{$1.58\%$}的结果超过$1\,\mathrm{km}$。在台站级系统偏差下，20台方案的三维误差95\%分位达到
\textbf{$986\,\mathrm{m}$}，超标比例为
\textbf{$3.3\%$}。因此，模型的关联层较为稳健，但定位精度仍明显依赖实际误差结构。
...
数据层面可联合模拟温度分层、风场、台站钟差、读数缺失、虚警及$1\sim2\,\mathrm{s}$粗差，并利用不同落区和不同监测台网布局进行交叉验证，避免模型阈值仅适用于单一的数据生成机制。
